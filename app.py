...
def add_tercero():
    ...
    try:
        ...
        print("Error agregando tercero:", e)
        return jsonify({"success": False, "error": str(e)})
...
def recibo_caja():
    ...
    try:
        ...
        print("Error cargando terceros:", e)
        terceros = []
    try:
        cur.execute("SELECT MAX(numero) as max_num FROM recibos_caja")
        result = cur.fetchone()
        last_num = result["max_num"] if result and result["max_num"] else 0
    except Exception as e:
        print("Error obteniendo último recibo:", e)
        last_num = 0
    conn.close()
...
def api_recibos_lista():
    ...
    try:
        ...
        print("Error listando recibos:", e)
        return jsonify({"success": False, "error": str(e)})
...
def comprobante_egreso():
    ...
    try:
        ...
        print("Error obteniendo último egreso:", e)
        last_num = 0
    conn.close()
...
def comprobante_egreso_save():
    ...
    try:
        ...
        print("Error guardando egreso:", e)
        return jsonify({"success": False, "error": str(e)})
...
def api_egresos_lista():
    ...
    try:
        ...
        print("Error listando egresos:", e)
        return jsonify({"success": False, "error": str(e)})
...
def add_cuenta():
    ...
    try:
        ...
        print("Error agregando cuenta:", e)
        # PostgreSQL usa un mensaje diferente para claves duplicadas
...
def seed_puc():
    ...
    try:
        ...
        print("Error inicializando PUC:", e)
        return jsonify({"success": False, "error": str(e)})
...
def api_resumen_ventas():
    ...
    try:
        ...
        cliente_top = cur.execute("""
            SELECT t.nombres || ' ' || COALESCE(t.apellidos,'') as cliente, COUNT(*) as num_compras, SUM(f.total) as total_comprado
...
def api_resumen_compras():
    ...
    try:
        ...
        proveedor_top = cur.execute("""
            SELECT t.nombres || ' ' || COALESCE(t.apellidos,'') as proveedor, COUNT(*) as num_compras, SUM(c.total) as total_comprado
        ...
        print("Error resumen compras:", e)
        return jsonify({"success": False, "error": str(e)})

@app.route("/api/facturas/lista", methods=["POST"])
def api_("/login", methods=["GET", "POST"])
def login():
...
def facturacion():
    ...
    cur = conn.cursor()
    try:
        # PostgreSQL: usar COALESCE en lugar de IFNULL
    ...
    try:
        ...
        print("Error obteniendo último número de factura:", e)
        last_num = 0
    conn.close()
...
def crear_nota_credito(factura_id):
    ...
    try:
        ...
        print("Error crear nota crédito:", e)
        conn.close()
        return redirect(url_for("facturas"))
...
def save_nota_credito():
    ...
    try:
        ...
        print("Error guardando nota crédito:", e)
        return redirect(url_for("crear_nota_credito", factura_id=factura_id))
...
def compras():
    ...
    cur = conn.cursor()
    try:
        proveedores = cur.execute("SELECT id, nombres || ' ' || COALESCE(apellidos,'') as nombre FROM terceros WHERE tipo='Proveedor'").fetchall()
    except Exception as e:
    ...
    try:
        ...
        result = cur.fetchone()
        last_num = result["max_id"] if result and result["max_id"] else 0
    except Exception as e:
        print("Error obteniendo último número de compra:", e)
        last_num = 0
    conn.close()
...
def compras_save():
    ...
    try:
        ...
        cur.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (proveedor_id, numero, fecha, total, forma_pago, True if forma_pago=="contado" else False))
